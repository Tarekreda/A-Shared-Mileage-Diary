...
app = Flask(__name__)
r= redis.Redis(host='redis', port=6379)             #setting the redis data base host and port inside the docker container


producer = KafkaProducer(bootstrap_servers=['kafka:9093'],api_version=(0, 10, 2))   #connecting the producer to the broker contained in the other docker container
...
def bill():
    ...
    if request.method == 'POST':  
        ...
        sum_user=0.0
        sum_total=0.0
        for x in range (int(r.get('number').decode())):   # the logic calculations needed for the project (very simple to demonstrate the idea)
